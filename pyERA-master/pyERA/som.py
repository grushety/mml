...
class Som:
    ...
    def return_BMU_index_cosine(self, input_vector):
        """Return the coordinates of the BMU.
        ...
        while not it.finished:
            #print "%d <%s>" % (it[0], it.multi_index)
            dist1 = self.return_euclidean_distance(input_vector, self._weights_matrix[it.multi_index[0], it.multi_index[1], :])
            dist2 = self.return_cosine_similarity(input_vector, self._weights_matrix[it.multi_index[0], it.multi_index[1], :])
            output_matrix[it.multi_index[0], it.multi_index[1]] = 0.5*(dist1 + 1-dist2)
            it.iternext()
        row, col = np.unravel_index(output_matrix.argmin(), output_matrix.shape)
        return (row, col)
